...
import sessionService from '../../services/piper/sessions/session.service';
import thresholdService from '../../services/piper/goals/threshold.service';
import soapService from '../../services/piper/reports/soap.service';
import pdfService from '../../services/piper/reports/pdf.service';
...
/**
 * POST /api/piper/child/session/start
 * Start a session for the logged-in child
 */
router.post('/child/session/start', authenticateChild, (req: Request, res: Response) => {
  try {
    const goalId = req.body.goal_id;

    // Get child to find therapist_id
    const child = childService.getChildById(req.child!.child_id);
    if (!child) {
      res.status(404).json({ error: 'Child not found' });
      return;
    ...
    const session = sessionService.startSession(
      child.therapist_id,
      {
        student_id: req.child!.child_id,
        iep_goal_id: goalId || undefined,
      }
    );
...
/**
 * POST /api/piper/child/session/trial
 * Record a trial for the child's current session
 */
router.post('/child/session/trial', authenticateChild, (req: Request, res: Response) => {
  try {
    const data: RecordTrialRequest = req.body;
    const sessionId = req.body.session_id;

    if (!sessionId || !data.card_category || !data.card_data || data.is_correct === undefined) {
      res.status(400).json({ error: 'session_id, card_category, card_data, and is_correct required' });
      return;
    }
    ...
    const trial = sessionService.recordTrial(
      sessionId,
      child.therapist_id,
      data
    );

    const accuracy = sessionService.getRunningAccuracy(sessionId);
    res.json({ trial, running_accuracy: accuracy });
  } catch (error) {
...
/**
 * POST /api/piper/child/session/end
 * End the child's current session
 */
router.post('/child/session/end', authenticateChild, async (req: Request, res: Response) => {
  try {
    const sessionId = req.body.session_id;
    ...
    const result = await sessionService.endSession(sessionId, child.therapist_id);
    res.json(result);
  } catch (error) {
...
/**
 * POST /api/piper/sessions/start
 * Start a new therapy session
 */
router.post('/sessions/start', authenticatePiper, (req: Request, res: Response) => {
  try {
...
/**
 * POST /api/piper/sessions/:id/trial
 * Record a trial result
 */
router.post('/sessions/:id/trial', authenticatePiper, (req: Request, res: Response) => {
  try {
    const data: RecordTrialRequest = req.body;

    if (!data.card_category || !data.card_data || data.is_correct === undefined) {
      res.status(400).json({ error: 'Card category, data, and is_correct required' });
      return;
    }
    ...
    const accuracy = sessionService.getRunningAccuracy(parseInt(req.params.id));
...
/**
 * PUT /api/piper/sessions/:id/end
 * End a session and generate AI SOAP note
 */
router.put('/sessions/:id/end', authenticatePiper, async (req: Request, res: Response) => {
  try {
    ...
    // Generate AI SOAP note for the completed session
    const generated_soap = await soapService.generateAISOAP(sessionId);
...
/**
 * PUT /api/piper/sessions/:id/soap
 * Save SOAP note for a session
 */
router.put('/sessions/:id/soap', authenticatePiper, (req: Request, res: Response) => {
  try {
    const { soap_subjective, soap_objective, soap_assessment, soap_plan } = req.body;

    if (!soap_subjective || !soap_objective || !soap_assessment || !soap_plan) {
      res.status(400).json({ error: 'All SOAP fields (S, O, A, P) are required' });
      return;
    }

    const session = sessionService.updateSessionSOAP(
      parseInt(req.params.id),
      req.therapist!.therapist_id,
      { soap_subjective, soap_objective, soap_assessment, soap_plan }
    );

    res.json({ session, message: 'SOAP note saved successfully' });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to save SOAP note';
    res.status(400).json({ error: message });
  }
});

/**
 * PUT /api/piper/sessions/:id/review
 * Mark session as reviewed by SLP (for export confirmation)
 */
router.put(
  '/sessions/:id/review',
  authenticatePiper,
  (req: Request, res: Response) => {
    try {
      ...
      if (typeof slp_reviewed !== 'boolean') {
        res.status(400).json({ error: 'slp_reviewed must be true or false' });
        return;
      }

      const session = sessionService.updateSessionReview(
        parseInt(req.params.id),
        req.therapist!.therapist_id,
        slp_reviewed
      );
      ...
      res.json({
        session,
        message: `Session marked as ${slp_reviewed ? 'reviewed' : 'not reviewed'}`,
      });
    } catch (error) {
      const message =
...
/**
 * DELETE /api/piper/sessions/:id
 * Cancel a session
 */
router.delete('/sessions/:id', authenticatePiper, (req: Request, res: Response) => {
  try {
    const success = sessionService.cancelSession(
      parseInt(req.params.id),
      req.therapist!.therapist_id
    );
...
/**
 * GET /api/piper/sessions/:id
 * Get session details with trials
 */
router.get('/sessions/:id', authenticatePiper, (req: Request, res: Response) => {
  try {
    const session = sessionService.getSessionWithTrials(
      parseInt(req.params.id),
      req.therapist!.therapist_id
    );
...
/**
 * GET /api/piper/students/:studentId/sessions
 * List sessions for a student
 */
router.get('/students/:studentId/sessions', authenticatePiper, (req: Request, res: Response) => {
  try {
    const limit = parseInt(req.query.limit as string) || 20;
    const sessions = sessionService.listSessionsByStudent(
      parseInt(req.params.studentId),
      req.therapist!.therapist_id,
      limit
    );
...
/**
 * GET /api/piper/sessions/:id/soap
 * Get SOAP note for a session
 */
router.get('/sessions/:id/soap', authenticatePiper, (req: Request, res: Response) => {
  try {
    // Verify access
    const session = sessionService.getSessionForTherapist(
      parseInt(req.params.id),
      req.therapist!.therapist_id
    );
    ...
    const soap = soapService.generateSessionSOAP(parseInt(req.params.id));

    if (!soap) {
      res.status(404).json({ error: 'Could not generate SOAP note' });
      return;
    }

    // Return as JSON or text based on Accept header
    if (req.headers.accept === 'text/plain') {
      res.type('text/plain').send(soapService.formatSessionSOAPAsText(soap));
    } else {
      res.json(soap);
    }
  } catch (error) {
...
/**
 * GET /api/piper/sessions/:id/soap/pdf
 * Download SOAP note as PDF
 */
router.get('/sessions/:id/soap/pdf', authenticatePiper, async (req: Request, res: Response) => {
  try {
    const sessionId = parseInt(req.params.id);

    // Verify access
    const session = sessionService.getSessionForTherapist(sessionId, req.therapist!.therapist_id);
    if (!session) {
      res.status(404).json({ error: 'Session not found' });
      return;
    }
    ...
    if (!pdfService.hasSOAPNote(sessionId)) {
      res.status(400).json({ error: 'No SOAP note saved for this session. Please save the SOAP note first.' });
      return;
    }

    // Get therapist name for the PDF
    const therapist = therapistService.getTherapistById(req.therapist!.therapist_id);
    const therapistName = therapist ? `${therapist.first_name} ${therapist.last_name}` : undefined;

    // Generate PDF
    const pdfBuffer = await pdfService.generateSOAPNotePDF(sessionId, therapistName);

    // Set headers for PDF download
    const student = studentService.getStudentById(session.student_id);
    const studentName = student ? `${student.first_name}_${student.last_name}` : 'Student';
    const date = new Date(session.start_time).toISOString().split('T')[0];
    const filename = `SOAP_Note_${studentName}_${date}.pdf`;
...
/**
 * GET /api/piper/goals/:id/pdf
 * Download IEP Goal as PDF
 */
router.get(
  '/goals/:id/pdf',
  authenticatePiper,
  async (req: Request, res: Response) => {
    try {
      ...
      if (!goal.slp_reviewed) {
        res.status(400).json({ error: 'Goal must be reviewed before downloading' });
        return;
      }

      const student = studentService.getStudentById(goal.student_id);
      if (!student) {
        res.status(404).json({ error: 'Student not found' });
        return;
      }
      ...
      // Generate PDF
      const pdfBuffer = await pdfService.generateGoalPDF(goalId, therapistName);

      // Set response headers
      const studentName = `${student.first_name}_${student.last_name}`.replace(
        /\s+/g,
        '_'
      );
      const filename = `IEP_Goal_${studentName}_${goal.goal_type}.pdf`;
      ...
      logger.error('Goal PDF generation error:', error);
      res.status(500).json({ error: message });
    }
  }
);
...
/**
 * GET /api/piper/goals/:id/soap
 * Get cumulative SOAP note for a goal
 */
router.get('/goals/:id/soap', authenticatePiper, (req: Request, res: Response) => {
  try {
    // Verify access
    const goal = goalService.getGoalForTherapist(
      parseInt(req.params.id),
    ...
    const soap = soapService.generateCumulativeSOAP(parseInt(req.params.id));
    ...
    // Return as JSON or text based on Accept header
    if (req.headers.accept === 'text/plain') {
      res.type('text/plain').send(soapService.formatCumulativeSOAPAsText(soap));
    } else {
      res.json(soap);
    }
  } catch (error) {
