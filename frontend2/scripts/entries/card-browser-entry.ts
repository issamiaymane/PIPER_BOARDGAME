...
function loadJsonModules(modules: Record<string, unknown>, fileSet: Set<string>): CategoryData {
    ...
    for (const path in modules) {
        ...
        // Extract filename from path (e.g., '../data/cards/language/vocabulary.json' -> 'vocabulary.json')
        const filename = path.split('/').pop() || 'unknown.json';
        fileSet.add(filename);
        if (content && typeof content === 'object') {
            for (const category in content) {
                // Track the source file for each category
...
function organizeByHandler(data: CategoryData): Record<string, { category: string; cards: CardData[]; handler: string; sourceFile: string }[]> {
    ...
    for (const category in data) {
        ...
        // Use explicit mapping first, then fall back to property-based detection
        let handlerType = CATEGORY_HANDLER_MAP[category];
        if (!handlerType) {
            // Fall back to standard for unmapped categories
    ...
    // Sort categories alphabetically within each handler
...
function renderSections() {
    ...
    for (const [handlerType, config] of Object.entries(HANDLERS)) {
        ...
        // Filter by search and source file
...
// ============================================================
// CARD PREVIEW RENDERING - Modular Handler-Based Architecture
// ============================================================

/**
 * Card Structure by Handler Type:
 * 1. single-answer: question + images[{image,label}] + answers[]
 * 2. multiple-answers: images + questions[{question, answer/answers}]
 * 3. multiple-choice: question + images + choices[] + answerIndex
 * 4. image-selection: question + images + answerIndex
 * 5. sequencing: question + images + answerSequence[]
 * 6. building: question + words[] + answerSequence[]
 * 7. conditional: question + images (action-based, no answer)
 *
 * Optional fields: title, story, paragraph, sentence
 */
...
function renderSequencing(card: CardData): string {
    ...
    // Images with click-to-order interface (like building handler but for images)
    if (card.images && Array.isArray(card.images)) {
        const images = normalizeImages(card.images as Array<string | { image?: string; label?: string }>);
...
function init() {
    ...
    // Populate source file filter with optgroups for Language and Articulation
...
// ============================================================
// EXPORTED FUNCTIONS - For use by other modules (e.g., boardgame)
// ============================================================
...
// Initialize on DOM ready - only if on card-browser page
